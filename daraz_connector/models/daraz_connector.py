...
class DarazConnector(models.Model):
    ...
    default_customer_id = fields.Many2one('res.partner', string='Default Daraz Customer',default="Administrator")
    state = fields.Selection([("draft", 'Draft'), ("connected", 'Connected')], default='draft')
    company_id = fields.Many2one('res.company', 'Company', required=True,
                                 default=lambda self: self.env.user.company_id.id)
    import_pending_orders = fields.Boolean(string='Import Only Pending Orders?',default=True)
    so_auto_import = fields.Boolean('Auto Import Sale Order?',default=True)
